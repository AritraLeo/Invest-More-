...
if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['g-recaptcha-response'] != "") {
    ...
    if ($responseData->success) {
        ...
        if (empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($phone_err) && empty($email_err)) {
            ...
            $sql = "INSERT INTO users (username, password, phone, email, address, refererusername, dt) VALUES (?, ?, ?, ?, ?, ?, current_timestamp())";
            $stmt = mysqli_prepare($conn, $sql);
            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "ssssss", $param_username, $param_password, $param_phone, $param_email, $param_address, $param_refusername);
